  ...
  def match    
    ...
    if (params.length <= 2)
      ...
      if(! params[:offer_cbox].nil? and ! params[:demand_cbox].nil?) # Both checkboxes are checked
        ...
        @services = Service.where('is_demand = :is_demand', :is_demand => false).where(:quick_match=>false)
      elsif(! params[:demand_cbox].nil?)
        @services = Service.where('is_demand = :is_demand', :is_demand => true).where(:quick_match=>false)
      else
